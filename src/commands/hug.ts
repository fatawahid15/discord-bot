...
export const data = new SlashCommandBuilder()
    .setName('hug')
    .setDescription('🫂 Spread the love with a hug!')
...
export const execute = async (interaction: ChatInputCommandInteraction) => {
    ...
    if (!targetUser) {
        await interaction.reply({ content: 'You must specify a user to hug.', ephemeral: true });
        return;
    }

    // Prevent users from hugging themselves
    if (author.id === targetUser.id) {
        await interaction.reply({ content: 'You can\'t hug yourself!', ephemeral: true });
        return;
    }
    ...
    const hugEmbed = new EmbedBuilder()
        ...
        .setDescription(`**${author} wraps ${targetUser} in a big hug!**`)
        .setImage(randomGif);
