...
export const data = new SlashCommandBuilder()
    .setName('slap')
    .setDescription('👋 Give another user a friendly slap! 👋')
...
export const execute = async (interaction: ChatInputCommandInteraction) => {
    ...
    if (!targetUser) {
        await interaction.reply({ content: 'You must specify a user to slap.', ephemeral: true });
        return;
    }

    // Prevent users from slapging themselves
    ...
    const slapEmbed = new EmbedBuilder()
        ...
        .setDescription(`**${author} gives ${targetUser} a big slap!**`)
        .setImage(randomGif);
